...
export default function connectionTemplatesRouter(db: PrismaClient): Router {
  const router = Router();
  ...
  router.delete("/:id", async (req: Request, res: Response) => {
    ...
    try {
      ...
      await db.connectionTemplate.delete({ where: { id } });
      res.status(204).end();
    } catch (err) {
      const e = err as { code?: string; message?: string };
      if (e.code === "P2025") return res.status(404).json({ error: "Connection Template not found" });
      logger.error({ err }, "connection template operation failed");
