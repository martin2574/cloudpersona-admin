...
interface DiffItem extends ResourceItem {
...
type ResourceKey = "categories" | "connectionTemplates" | "skillTemplates";

// 비교 대상 필드 (timestamps 제외)
const COMPARE_FIELDS: Record<ResourceKey, string[]> = {
...
const UPSERT_FIELDS: Record<ResourceKey, string[]> = {
  categories: ["name", "icon", "sortOrder"],
  connectionTemplates: [
    "serviceType", "name", "description", "version",
    "categoryId", "icon", "spec", "deprecatedAt",
 ...
 * 리소스 1종 diff 계산
 */
function diffResource(sourceItems: ResourceItem[], targetItems: ResourceItem[], fields: string[]): DiffResult {
  const targetMap = new Map(targetItems.map((item) => [item.id, item]));
  const create: DiffItem[] = [];
  const update: DiffItem[] = [];
 ...
 */
function buildUpsertBody(item: ResourceItem, fields: string[]): Record<string, unknown> {
 ...
 */
async function fetchTarget(apiServerUrl: string, adminSecret: string, resource: string): Promise<ResourceItem[]> {
  ...
  return resp.json() as Promise<ResourceItem[]>;
}

/**
 * API Server에 PUT upsert
 */
async function upsertTarget(apiServerUrl: string, adminSecret: string, resource: string, id: string, body: Record<string, unknown>): Promise<unknown> {
 ...
 * API Server에서 리소스 삭제
 */
async function deleteTarget(apiServerUrl: string, adminSecret: string, resource: string, id: string): Promise<void> {
  const url = `${apiServerUrl}/api/internal/admin/${resource}/${id}`;
  const resp = await fetch(url, {
    method: "DELETE",
    headers: {
 ...
 */
export async function reconcile(db: PrismaClient, { apiServerUrl, adminSecret, mode }: ReconcileOptions) {
  ...
  // 3. Diff 계산
  const diff: Record<ResourceKey, DiffResult> = {
    categories: diffResource(srcCategories as ResourceItem[], tgtCategories, COMPARE_FIELDS.categories),
    connectionTemplates: diffResource(srcConnections as ResourceItem[], tgtConnections, COMPARE_FIELDS.connectionTemplates),
    skillTemplates: diffResource(srcSkills as ResourceItem[], tgtSkills, COMPARE_FIELDS.skillTemplates),
  };

  const summary: Record<string, { create: number; update: number; skip: number; delete: number }> = {};
  ...
  // dry-run: diff만 반환
  if (mode === "dry-run") {
    return { mode: "dry-run" as const, diff, summary };
  }

  // 4. Execute: FK 순서대로 upsert (categories → connectionTemplates → skillTemplates)
  const RESOURCE_ORDER: { key: ResourceKey; resource: string; fields: string[] }[] = [
  ...
  const results: Record<ResourceKey, SyncResult[]> = { categories: [], connectionTemplates: [], skillTemplates: [] };
  let stopped = false;
  ...
  const DELETE_ORDER: { key: ResourceKey; resource: string }[] = [
  ...
  for (const { key, resource } of DELETE_ORDER) {
    ...
    for (const item of diff[key].delete) {
      try {
        ...
        results[key].push({ id: item.id, name: item.name as string, action: "delete", success: true });
      } catch (err) {
        logger.warn({ err, resource, id: item.id }, "reconcile item delete failed");
        results[key].push({ id: item.id, name: item.name as string, action: "delete", success: false, error: (err as Error).message });
        stopped = true;
        break;
      }
    }
  ...
  const executeSummary: Record<string, { success: number; failed: number }> = {};
  ...
  return { mode: "execute" as const, results, summary: executeSummary, stopped };
}
