...
const ENV_LABELS: Record<string, string> = { test: "Test", prod: "Production" };
const ENV_COLORS: Record<string, string> = {
  test: "bg-blue-100 text-blue-800",
  prod: "bg-red-100 text-red-800",
};

interface SummaryCardProps extends ReconcileSummary {
  label: string;
}

function SummaryCard({ label, create, update, skip, delete: del }: SummaryCardProps) {
  return (
    <Card>
...
function DiffDetail({ title, diff }: DiffDetailProps) {
  if (!diff) return null;
  const hasCreate = (diff.create?.length ?? 0) > 0;
  const hasUpdate = (diff.update?.length ?? 0) > 0;
  const hasDelete = (diff.delete?.length ?? 0) > 0;
...
interface ExecuteResultsProps {
  data: ReconcileExecuteResult | null;
}

function ExecuteResults({ data }: ExecuteResultsProps) {
  if (!data) return null;
                    ...
                    <td className="py-2 px-3">
                      {item.success ? (
                        ...
                        <span className="flex items-center gap-1 text-destructive">
                          <XCircle className="h-4 w-4" /> {item.error}
                        </span>
                      )}
                    </td>
                  </tr>
...
export default function Reconcile() {
  ...
  const [dryRunResult, setDryRunResult] = useState<ReconcileDryRunResult | null>(null);
  const [executeResult, setExecuteResult] = useState<ReconcileExecuteResult | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    getReconcileEnvs()
      .then((list) => {
        const envList = (list ?? []) as string[];
        setEnvs(envList);
        if (envList.length > 0) setSelectedEnv(envList[0]);
      })
      .catch(console.error);
  }, []);
  ...
  const handleDryRun = async () => {
    ...
    try {
      const result = (await reconcileDryRun(selectedEnv)) as ReconcileDryRunResult;
  ...
  const handleExecute = async () => {
    ...
    if (
      !window.confirm(
        `Backoffice 데이터를 ${envLabel} 환경에 동기화합니다. 계속하시겠습니까?`,
      )
    )
      return;
    setLoading(true);
    setError(null);
    try {
      const result = (await reconcileExecute(selectedEnv)) as ReconcileExecuteResult;
  ...
  const hasChanges =
    dryRunResult &&
    Object.values(dryRunResult.summary).some(
      ...
      {dryRunResult && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            ...
            <SummaryCard
              label="Connection Templates"
              {...dryRunResult.summary.connectionTemplates}
            />
            <SummaryCard
              label="Skill Templates"
              {...dryRunResult.summary.skillTemplates}
            />
          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <DiffDetail
                  ...
                  diff={dryRunResult.diff.connectionTemplates}
                />
                <DiffDetail
                  title="Skill Templates"
                  diff={dryRunResult.diff.skillTemplates}
                />
              </CardContent>
            </Card>
          ) : (
