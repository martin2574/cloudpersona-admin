...
describe("TC-007.3-12: execute delete FK 역순 보장", () => {
  it("skill-templates → connection-templates → categories 순서로 DELETE 호출한다", async () => {
    ...
    // 전체 3건 삭제 성공
    expect(result.summary.skillTemplates.success).toBe(1);
    expect(result.summary.connectionTemplates.success).toBe(1);
  ...
  it("DELETE 실패 시 즉시 멈추고 stopped=true를 반환한다", async () => {
    ...
    expect(result.summary.skillTemplates.failed).toBe(1);
