...
describe("TC-007.3-11: dry-run delete diff 정확성", () => {
  it("Target에만 있는 항목을 delete 배열에 포함한다", async () => {
    ...
    // delete diff 확인
    const s = result.summary as Record<string, { create: number; update: number; skip: number; delete: number }>;
    expect(s.categories.delete).toBe(1);
    expect(result.diff!.categories.delete).toHaveLength(1);
    expect(result.diff!.categories.delete[0].id).toBe("cat-orphan");

    // 나머지 분류 확인
    expect(s.categories.skip).toBe(1);    // cat-1
    expect(s.categories.create).toBe(0);
    expect(s.categories.update).toBe(0);
  });
  ...
  it("Source와 Target이 동일하면 delete는 0이다", async () => {
    ...
    const s = result.summary as Record<string, { create: number; update: number; skip: number; delete: number }>;
    expect(s.categories.delete).toBe(0);
  ...
  it("dry-run 시 DELETE 호출이 0건이다", async () => {
    ...
    await reconcile(mockDb as unknown as PrismaClient, {
      apiServerUrl: "http://test:3057",
      adminSecret: "secret",
      mode: "dry-run",
    });
